...
export default function RichEditor({
            ...
            handlePaste(view, event) {
                ...
                void upload(file)
                    ...
                    .catch(err => {
                        ...
                        // Surface the error via the global window event so the page-level toast picks it up.
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0">
            ...
            <div className="rich-editor-root flex-1 min-h-0 overflow-y-auto px-6 py-4" style={accentColor ? { caretColor: accentColor } : undefined}>
...
function Toolbar({ editor, onUploadImage }: { editor: Editor; onUploadImage?: (file: File) => Promise<string> }) {
    ...
    const btn = (active: boolean) =>
        `px-2 py-1.5 text-[11px] font-bold transition rounded ${active ? "bg-white/15 text-white" : "text-zinc-400 hover:text-white hover:bg-white/5"}`;

    return (
        <div className="flex items-center gap-0.5 flex-wrap px-3 py-1.5 border-b border-white/[0.06] bg-black/20 flex-shrink-0">
