...
describe("isLocal", () => {
    ...
    describe("public hosts (must NOT be local)", () => {
        ...
        it("rejects arbitrary public IPs", () => {
            expect(isLocal(req("http://192.0.2.1/"))).toBe(false);
        });
...
// ── lanTrusted / isLocalHostname (hub production build over the LAN) ──
import { lanTrusted, hubLanTrusted, isLocalHostname } from "@/lib/is-local";
import { afterEach, vi } from "vitest";

describe("isLocalHostname", () => {
    it("accepts loopback, LAN ranges and *.localhost", () => {
        for (const h of ["localhost", "127.0.0.1", "10.0.0.2", "192.168.1.9", "stickies.localhost"]) expect(isLocalHostname(h)).toBe(true);
    });
    it("rejects public hosts", () => {
        for (const h of ["stickies-bheng.vercel.app", "example.com", "8.8.8.8"]) expect(isLocalHostname(h)).toBe(false);
    });
});

describe("lanTrusted", () => {
    afterEach(() => vi.unstubAllEnvs());
    it("passes a LAN request outside production", () => {
        vi.stubEnv("NODE_ENV", "development");
        expect(lanTrusted(req("http://10.0.0.2:4444/"))).toBe(true);
    });
    it("REFUSES a LAN-looking request in production without the hub opt-in", () => {
        vi.stubEnv("NODE_ENV", "production"); vi.stubEnv("STICKIES_LAN_TRUST", "");
        expect(lanTrusted(req("http://x.invalid/", "localhost"))).toBe(false);
    });
    it("passes a LAN request in production when the hub opts in", () => {
        vi.stubEnv("NODE_ENV", "production"); vi.stubEnv("STICKIES_LAN_TRUST", "1");
        expect(lanTrusted(req("http://x.invalid/", "stickies.localhost"))).toBe(true);
    });
    it("never passes a public host, even with the opt-in", () => {
        vi.stubEnv("NODE_ENV", "production"); vi.stubEnv("STICKIES_LAN_TRUST", "1");
        expect(lanTrusted(req("https://stickies-bheng.vercel.app/"))).toBe(false);
    });
});

describe("hubLanTrusted (opt-in only, used by the owner-browser bypass)", () => {
    afterEach(() => vi.unstubAllEnvs());
    it("is false without the opt-in even outside production", () => {
        vi.stubEnv("NODE_ENV", "test"); vi.stubEnv("STICKIES_LAN_TRUST", "");
        expect(hubLanTrusted(req("http://localhost:4444/"))).toBe(false);
    });
    it("is true with the opt-in on a local host, false on a public host", () => {
        vi.stubEnv("STICKIES_LAN_TRUST", "1");
        expect(hubLanTrusted(req("http://localhost:4444/"))).toBe(true);
        expect(hubLanTrusted(req("https://stickies-bheng.vercel.app/"))).toBe(false);
    });
});
