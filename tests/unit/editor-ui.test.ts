...
// The Cmd-K helpers read only a few permissive fields; these fixtures carry the
// minimal shape each one uses. Cast to NoteRow[] so tsconfig.test doesn't demand
// every column on a throwaway fixture (ids are intentionally mixed str/number).
const rows = (r: Array<Record<string, unknown>>): NoteRow[] => r as unknown as NoteRow[];
...
describe("mergeRecentNotes", () => {
    ...
    it("dedupes when the note also arrived live (no double row), incoming wins", () => {
        ...
        expect((out.find((n) => n.id === "b") as { title: string }).title).toBe("fresh title");
    });
    ...
    it("never drops folders and keeps optimistic notes not yet on the server", () => {
        ...
        expect((out.find((n) => n.id === "a") as { title: string }).title).toBe("A refreshed");
    });
...
describe("buildCmdKIndex", () => {
    it("drops folders and sorts notes by updated_at descending", () => {
        const idx = buildCmdKIndex(rows([
            { id: 1, is_folder: true, folder_name: "F" },
            { id: 2, title: "Old", updated_at: "2026-01-01" },
            { id: 3, title: "New", updated_at: "2026-06-01" },
        ]));
        expect(idx.map(e => e._orig.id)).toEqual([3, 2]);
        expect(idx[0].t).toBe("new");
    });

    it("lowercases and truncates content to 400 chars", () => {
        const [entry] = buildCmdKIndex(rows([{ id: 1, title: "T", content: "A".repeat(500), updated_at: "x" }]));
        expect(entry.c).toBe("a".repeat(400));
    });
});

describe("buildFolderLookup", () => {
    it("maps folder_name (or name) to the row, ignoring non-folders", () => {
        const map = buildFolderLookup(rows([
            { id: 1, is_folder: true, folder_name: "Work" },
            { id: 2, is_folder: true, name: "Home" },
            { id: 3, title: "note" },
        ]));
        expect(map.get("Work")?.id).toBe(1);
        expect(map.get("Home")?.id).toBe(2);
        expect(map.has("note")).toBe(false);
    });
});

describe("computeCmdKResults", () => {
    const base = (over: Partial<Parameters<typeof computeCmdKResults>[0]> = {}) => ({
        cmdKIndex: [],
    ...
    it("in-file mode returns matching lines with 1-based line numbers", () => {
        const res = computeCmdKResults(base({
            cmdKInFile: true,
            deferredCmdKQuery: "todo",
            deferredContent: "first\nsecond TODO\nthird",
        }));
        expect(res).toHaveLength(1);
        expect(res[0]).toMatchObject({ _isLine: true, lineNum: 2, text: "second TODO" });
    });

    it("empty query returns the 5 most-recently-CREATED notes", () => {
        const idx = buildCmdKIndex(rows(Array.from({ length: 40 }, (_, i) => ({
            id: i, title: `n${i}`, updated_at: `2026-01-01`, created_at: `2026-02-${String((i % 28) + 1).padStart(2, "0")}`,
        }))));
        const res = computeCmdKResults(base({ cmdKIndex: idx }));
        expect(res).toHaveLength(5);
        // newest created_at first
        expect((res[0] as any).created_at.localeCompare((res[1] as any).created_at)).toBeGreaterThan(0);
    });

    it("orders matches by recency (updated_at desc, then created_at desc), not match tier", () => {
        // Search results float the most-recently-touched note to the top. The match
        // tier only gates what qualifies; it no longer decides order — so a newer
        // body-only match now sits above an older exact-title match.
        const idx = buildCmdKIndex(rows([
            { id: "body", title: "unrelated", content: "mentions vincent here", updated_at: "2026-06-02", created_at: "2026-01-01" },
            { id: "title", title: "Vincent invoice", content: "", updated_at: "2026-06-01", created_at: "2026-01-01" },
        ]));
        const res = computeCmdKResults(base({ cmdKIndex: idx, deferredCmdKQuery: "vincent" }));
        expect(res.map(r => r.id)).toEqual(["body", "title"]);
    });

    it("ties on updated_at broken by created_at desc", () => {
        const idx = buildCmdKIndex(rows([
            { id: "older", title: "vincent a", updated_at: "2026-06-02", created_at: "2026-01-01" },
            { id: "newer", title: "vincent b", updated_at: "2026-06-02", created_at: "2026-05-01" },
        ]));
        const res = computeCmdKResults(base({ cmdKIndex: idx, deferredCmdKQuery: "vincent" }));
        expect(res.map(r => r.id)).toEqual(["newer", "older"]);
    });

    it("puts folder matches AFTER notes (bottom) and skips BOOKMARKS", () => {
        const idx = buildCmdKIndex(rows([{ id: "n", title: "work log", updated_at: "2026-06-01" }]));
        const res = computeCmdKResults(base({
            cmdKIndex: idx,
            folders: [{ name: "Work" }, { name: "BOOKMARKS" }],
            deferredCmdKQuery: "work",
        }));
        expect(res[0]).toMatchObject({ id: "n" });                                    // notes first
        expect(res[res.length - 1]).toMatchObject({ _isFolder: true, name: "Work" }); // folder pinned to the bottom
        expect(res.some(r => r.name === "BOOKMARKS")).toBe(false);
    });

    it("merges server hits not already in the local index", () => {
        const res = computeCmdKResults(base({
            deferredCmdKQuery: "remote",
            cmdKServerResults: rows([{ id: "srv", title: "remote note", updated_at: "2026-06-01" }]),
        }));
        expect(res.map(r => r.id)).toContain("srv");
    });
});

describe("noteOrderByUpdated / noteOrderByCreated", () => {
    const A = { id: "a", title: "Apple", updated_at: "2026-07-01T00:00:00Z", created_at: "2026-01-01T00:00:00Z" };
    const B = { id: "b", title: "Banana", updated_at: "2026-07-02T00:00:00Z", created_at: "2026-06-01T00:00:00Z" };
    it("byUpdated: newest updated_at first", () => {
        expect([A, B].sort(noteOrderByUpdated).map(n => n.id)).toEqual(["b", "a"]);
    });
    it("byCreated: newest created_at first (edit does not reorder)", () => {
        // B was edited later but A/B created order: B created 2026-06 > A 2026-01, so B first
        expect([A, B].sort(noteOrderByCreated).map(n => n.id)).toEqual(["b", "a"]);
        // an older-created note stays below even if just edited
        const oldCreatedFreshEdit = { id: "c", title: "C", updated_at: "2026-07-09T00:00:00Z", created_at: "2025-01-01T00:00:00Z" };
        expect([oldCreatedFreshEdit, A].sort(noteOrderByCreated).map(n => n.id)).toEqual(["a", "c"]);
    });
    it("_pinnedToTop wins over timestamp in both", () => {
        const pinned = { id: "p", title: "Z", updated_at: "2020-01-01T00:00:00Z", created_at: "2020-01-01T00:00:00Z", _pinnedToTop: true };
        expect([A, pinned].sort(noteOrderByUpdated)[0].id).toBe("p");
        expect([A, pinned].sort(noteOrderByCreated)[0].id).toBe("p");
    });
    it("ties broken by title", () => {
        const x = { id: "x", title: "Zebra", updated_at: "2026-07-05T00:00:00Z", created_at: "2026-07-05T00:00:00Z" };
        const y = { id: "y", title: "Ant", updated_at: "2026-07-05T00:00:00Z", created_at: "2026-07-05T00:00:00Z" };
        expect([x, y].sort(noteOrderByUpdated).map(n => n.title)).toEqual(["Ant", "Zebra"]);
    });
});

describe("computeDisplayItems", () => {
    const base = { folderStack: [], currentLevelFolders: [{ id: "F", is_folder: true, name: "Fld" }], pinnedIds: new Set<string>(), todayNotes: [], todayDays: 1, showFileIcons: false };
    const note = (o: any) => ({ is_folder: false, trashed_at: null, folder_name: "CLAUDE", ...o });

    it("root (no folder, no search) returns the folder grid", () => {
        const r = computeDisplayItems({ ...base, dbData: [note({ id: "a" })], activeFolder: null, search: "", allNotes: [] });
        expect(r.map((x: any) => x.id)).toEqual(["F"]);
    });
    it("search ranks folder-name over title over content, drops non-matches", () => {
        const db = [
            note({ id: "t", title: "alpha widget", content: "x" }),
            note({ id: "c", title: "z", content: "has alpha inside" }),
            note({ id: "n", title: "nope", content: "nope" }),
        ];
        const r = computeDisplayItems({ ...base, dbData: db, activeFolder: "CLAUDE", search: "alpha", allNotes: [] });
        expect(r.map((x: any) => x.id)).toEqual(["t", "c"]); // title-match before content-match, "nope" dropped
    });
    it("All view orders by created_at and excludes TRASH", () => {
        const allNotes = [
            note({ id: "new", created_at: "2026-07-05T00:00:00Z", updated_at: "2026-07-05T00:00:00Z" }),
            note({ id: "old", created_at: "2026-07-01T00:00:00Z", updated_at: "2026-07-09T00:00:00Z" }),
        ];
        const r = computeDisplayItems({ ...base, dbData: [], activeFolder: "All", search: "", allNotes });
        expect(r.filter((x: any) => !x.is_folder).map((x: any) => x.id)).toEqual(["new", "old"]);
    });
    it("folder view returns only that folder's live notes", () => {
        const db = [
            note({ id: "in", folder_name: "Work" }),
            note({ id: "out", folder_name: "Home" }),
            note({ id: "trash", folder_name: "Work", trashed_at: "2026-01-01T00:00:00Z" }),
        ];
        const r = computeDisplayItems({ ...base, dbData: db, activeFolder: "Work", search: "", allNotes: [] });
        expect(r.filter((x: any) => !x.is_folder).map((x: any) => x.id)).toEqual(["in"]);
    });
    it("showFileIcons puts folders first, else notes first", () => {
        const db = [note({ id: "a", folder_name: "Work" })];
        const off = computeDisplayItems({ ...base, showFileIcons: false, dbData: db, activeFolder: "Work", search: "", allNotes: [] });
        const on = computeDisplayItems({ ...base, showFileIcons: true, dbData: db, activeFolder: "Work", search: "", allNotes: [] });
        expect(off[0].id).toBe("a");   // notes first
        expect(on[0].id).toBe("F");    // folders first
    });
});
