...
describe("authorizeOwner", () => {
    ...
    describe("production Host-header hardening", () => {
        afterEach(() => vi.unstubAllEnvs());

        it("does NOT grant the local bypass in production (a spoofed Host cannot authenticate)", async () => {
            vi.stubEnv("NODE_ENV", "production");
            mockAuth.mockResolvedValue(null); // no owner session either
            const req = new Request("http://localhost:4444/api/test"); // spoofable Host header
            expect(await authorizeOwner(req)).toBe(false);
        });

        it("still accepts a valid API key in production", async () => {
            vi.stubEnv("NODE_ENV", "production");
            const req = new Request("http://localhost:4444/api/test", {
    ...
    describe("production key lockdown (VERCEL_ENV) - blocks the shared static key on sub-routes", () => {
        afterEach(() => vi.unstubAllEnvs());

        it("rejects the shared static API key on Vercel production", async () => {
            vi.stubEnv("VERCEL_ENV", "production");
            mockAuth.mockResolvedValue(null); // no owner session
            const req = new Request("https://stickies.example.com/api/stickies/backup", {
                headers: { Authorization: `Bearer ${API_KEY}` },
            });
            expect(await authorizeOwner(req)).toBe(false);
        });

        it("rejects the static password on Vercel production", async () => {
            vi.stubEnv("VERCEL_ENV", "production");
        ...
        it("still accepts the static key when NOT on Vercel prod (local/LAN unaffected)", async () => {
            const req = new Request("https://stickies.example.com/api/stickies/backup", {
                headers: { Authorization: `Bearer ${API_KEY}` },
            });
            expect(await authorizeOwner(req)).toBe(true);
