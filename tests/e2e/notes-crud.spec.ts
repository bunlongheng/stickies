...
test.describe("Notes CRUD", () => {
    ...
    test("create a new note via the FAB then clean it up", async ({ page, request }) => {
        ...
        // WebKit can resolve the click before focus has left the title input, which
        // sends the body text into the title field. Wait for focus to actually land
        // inside the editor surface before typing.
        await expect
            .poll(() => page.evaluate(() => !!document.activeElement?.closest(".rich-editor-prose")), { timeout: 10_000 })
            .toBe(true);
