...
test.describe("Rich editor (Phase 1)", () => {
    test("sign-in page renders the Stickies card", async ({ page }) => {
        ...
        // The card offers exactly one way in, and which one depends on the BUILD, not
        // the spec: `next dev` renders the "Go in (dev)" bypass, a production build
        // renders the Google button. The suite reuses whatever server holds port 4444
        // (see playwright.config webServer.reuseExistingServer), which on the hub is
        // the production build - so assert the card has a way in, not which one.
        const devBypass = page.getByText("Go in (dev)");
        const googleButton = page.getByRole("button", { name: /google/i });
        await expect(devBypass.or(googleButton).first()).toBeVisible();
    });

    test("home renders folder grid or list", async ({ page }) => {
        await page.goto("/");
        const anyContent = page.locator("[data-note-id]");
