...
// A valid CSRF state present in BOTH the query and the cookie (the happy path).
const STATE = "a".repeat(64);
const REQ = (qs: string) => {
    const sep = qs ? "&" : "?";
    return new NextRequest(`http://localhost:4444/api/stickies/gdrive/callback${qs}${sep}state=${STATE}`, {
        headers: { cookie: `gdrive_oauth_state=${STATE}` },
    });
};

describe("GET /api/stickies/gdrive/callback", () => {
    it("rejects with bad_state when the state cookie is missing or mismatched", async () => {
        const { GET } = await import("@/app/api/stickies/gdrive/callback/route");
        // query state present but no matching cookie
        const res = await GET(new NextRequest("http://localhost:4444/api/stickies/gdrive/callback?code=x&state=zzz"));
        const loc = res.headers.get("location") ?? "";
        expect(loc).toContain("gdrive=error");
        expect(loc).toContain("bad_state");
    });
