 ...
 *   DB-null 500, doc JSONB stringify
...
import { ENV, API_KEY, USER_ID, apiReq, noAuthReq, noteRow, json } from "./helpers";
...
describe("Single-note upsert broadcast", () => {
    // Resurface is ON by default: a re-post should surface as a fresh arrival so it
    // ranks to the top of the created_at-ordered All view. So an existing-note update
    // broadcasts note-created (the live add path), not note-updated.
    it("broadcasts note-created on a default (resurfacing) re-post of an existing note", async () => {
    ...
    // Opt out with resurface:false → a silent edit: note-updated, created_at untouched.
    it("resurface:false keeps note-updated and leaves created_at alone", async () => {
        mockQuery.mockResolvedValue([]);
        mockQueryOne
            .mockResolvedValueOnce({ id: "existing-2" })
            .mockResolvedValueOnce(noteRow({ id: "existing-2", title: "Dupe" }));
        await POST(apiReq("/api/stickies", {
            method: "POST",
            body: { title: "Dupe", content: "updated", folder_name: "Work", resurface: false },
            headers: { "User-Agent": "Mozilla/5.0" },
        }));
        expect(mockPusherTrigger).toHaveBeenCalledWith("stickies", "note-updated", expect.anything());
        const updateSql = mockQueryOne.mock.calls.map(c => String(c[0])).find(q => q.includes("UPDATE"));
        expect(updateSql).toBeTruthy();
        expect(updateSql).not.toContain("created_at");
    });

    // Default re-post bumps created_at so the regenerated report ranks as new.
    it("upsert bumps created_at by default (resurface on)", async () => {
        mockQuery.mockResolvedValue([]);
        mockQueryOne
            .mockResolvedValueOnce({ id: "keep-1" })
            .mockResolvedValueOnce(noteRow({ id: "keep-1", title: "Report" }));
        await POST(apiReq("/api/stickies", {
            method: "POST",
            body: { title: "Report", content: "v2", folder_name: "Work" },
            headers: { "User-Agent": "Mozilla/5.0" },
        }));
        const updateSql = mockQueryOne.mock.calls.map(c => String(c[0])).find(q => q.includes("UPDATE"));
        expect(updateSql).toBeTruthy();
        expect(updateSql).toContain("created_at");
    });

    it("upsert with resurface:true bumps created_at so the report ranks as new", async () => {
        mockQuery.mockResolvedValue([]);
        mockQueryOne
            .mockResolvedValueOnce({ id: "bump-1" })
            .mockResolvedValueOnce(noteRow({ id: "bump-1", title: "Report" }));
        await POST(apiReq("/api/stickies", {
            method: "POST",
            body: { title: "Report", content: "v2", folder_name: "Work", resurface: true },
            headers: { "User-Agent": "Mozilla/5.0" },
        }));
        const updateSql = mockQueryOne.mock.calls.map(c => String(c[0])).find(q => q.includes("UPDATE"));
        expect(updateSql).toContain("created_at");
...
describe("DELETE TRASH folder", () => {
    it("deletes only notes inside TRASH, never the folder row", async () => {
        // DELETE is owner-browser-only: API keys get 403, so authenticate as the owner.
        mockAuth.mockResolvedValue({ user: { id: USER_ID, email: process.env.OWNER_EMAIL } });
        mockExecute.mockResolvedValue(3);
        const res = await DELETE(noAuthReq("/api/stickies?folder_name=TRASH", "DELETE"));
        expect(res.status).toBe(200);
        const sql = mockExecute.mock.calls.map(c => c[0] as string).find(s => /DELETE/i.test(s)) ?? "";
...
describe("PATCH DB error", () => {
    it("returns 500 when the UPDATE query throws", async () => {
        mockQueryOne
            .mockResolvedValueOnce({ locked: false }) // lock-check (SELECT locked) succeeds
            .mockRejectedValueOnce(new Error("column \"foo\" does not exist")); // UPDATE throws
