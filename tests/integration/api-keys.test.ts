...
describe("note attribution via per-machine key", () => {
    ...
    it("legacy static key still creates a note (attributed to the calling machine, the hub by default)", async () => {
        // The static key is labeled 'legacy' in _auth, but the route prefers the
        // calling machine for attribution (route.ts createdByKey resolution).
        // With no forwarded-for headers the request resolves to the hub, whose name
        // is deployment config (NEXT_PUBLIC_STICKIES_HUB_MACHINE).
        ...
        mockQueryOne
            ...
            .mockResolvedValueOnce({ id: "n2", title: "Legacy", folder_color: "#FF3B30", created_by_key: "Hub" }); // INSERT

        const res = await NOTE_POST(noteReq("test-api-key"));
        expect(res.status).toBe(201);
        const insertCall = mockQueryOne.mock.calls.find(c => String(c[0]).includes("INSERT INTO \"stickies\""))!;
        expect(String(insertCall[0])).toContain("created_by_key");
        expect(insertCall[1] as unknown[]).toContain("Hub");
        vi.unstubAllEnvs();
    });
});
