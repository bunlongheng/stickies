...
// The route is owner-only; treat the caller as authorized for these behavior tests.
vi.mock("@/app/api/stickies/_auth", () => ({ authorizeOwner: vi.fn().mockResolvedValue(true) }));

// Authorized request passed to the handler (auth is mocked above).
const REQ = () => new Request("http://localhost:4444/api/hue/groups");
...
describe("GET /api/hue/groups", () => {
    ...
    it("maps grouped_light IDs to parent room name (remote path)", async () => {
        mockQueryOne.mockResolvedValueOnce({
            id: "int-1",
            config: { app_key: "appkey", client_id: "cid", client_secret: "secret", bridge_ip: "10.0.0.2" },
    ...
    it("returns 401 when the caller is not the owner (external hit on the deployed origin)", async () => {
        const { authorizeOwner } = await import("@/app/api/stickies/_auth");
        vi.mocked(authorizeOwner).mockResolvedValueOnce(false);
        const { GET } = await import("@/app/api/hue/groups/route");
        const res = await GET(REQ());
