...
describe("GET /api/stickies/gdrive/auth", () => {
    it("redirects to Google OAuth with correct params (owner-authed) + a CSRF state cookie", async () => {
        const { GET } = await import("@/app/api/stickies/gdrive/auth/route");
        const res = await GET(new Request("http://localhost:4444/api/stickies/gdrive/auth"));
        ...
        // the same state is pinned into an httpOnly cookie for the callback to verify
        expect(res.headers.get("set-cookie") || "").toMatch(/gdrive_oauth_state=[a-f0-9]{64}/);
    });

    it("401s when the caller is not the owner", async () => {
        const authMod = await import("@/app/api/stickies/_auth");
        vi.mocked(authMod.authorizeOwner).mockResolvedValueOnce(false);
        const { GET } = await import("@/app/api/stickies/gdrive/auth/route");
        const res = await GET(new Request("http://localhost:4444/api/stickies/gdrive/auth"));
...
// Valid CSRF state in both query and cookie - the happy path.
const CB_STATE = "b".repeat(64);
const cbReq = (qs: string) =>
    new NextRequest(`http://localhost:4444/api/stickies/gdrive/callback${qs}${qs ? "&" : "?"}state=${CB_STATE}`, {
        headers: { cookie: `gdrive_oauth_state=${CB_STATE}` },
    });
...
describe("GET /api/stickies/gdrive/callback", () => {
    ...
    it("redirects with error when error param present", async () => {
        const { GET } = await import("@/app/api/stickies/gdrive/callback/route");
        const res = await GET(cbReq("?error=access_denied"));
