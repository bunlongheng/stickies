 ...
 *  - `type` column (auto-detected): text, html, json,
 *    code-javascript, code-typescript, code-python, code-css, code-sql, code-bash, voice
 *  - `format` column (Phase 1): text, rich  (markdown was dropped 2026-07-27)
 *
...
const TYPE_CASES: { name: string; type: string; content: string }[] = [
    ...
    // NOTE: markdown was dropped (2026-07-27) — notes are plain text or HTML only.
...
// ─── html is accepted from external channels (markdown was dropped 2026-07-27) ──
describe("POST /api/stickies — accepts html from external (API/CLI/MCP)", () => {
...
// ─── format flag (Phase 1) — rich + text round-trip ─────────────────────────
describe.each([
...
describe("checklist mode flag", () => {
    it("PATCH accepts list_mode=true", async () => {
        mockQueryOne
            .mockResolvedValueOnce({ locked: false })                 // lock-check (SELECT locked)
            .mockResolvedValueOnce({ id: "uuid-1", list_mode: true }); // UPDATE ... RETURNING *
        const { PATCH } = await import("@/app/api/stickies/ext/route");
        const res = await PATCH(apiReq("/api/stickies/ext", {
            method: "PATCH",
        ...
        expect(res.status).toBe(200);
        // The lock-check SELECT runs first; assert against the UPDATE call.
...
describe("JSONB doc binding (Phase 1 regression)", () => {
    it("stringifies doc JSON when sent via PATCH so node-pg serializes correctly", async () => {
        ...
        mockQueryOne
            .mockResolvedValueOnce({ locked: false })                        // lock-check (SELECT locked)
            .mockResolvedValueOnce({ id: "uuid-1", doc, format: "rich" });   // UPDATE ... RETURNING *
        const { PATCH } = await import("@/app/api/stickies/ext/route");
        await PATCH(apiReq("/api/stickies/ext", {
            method: "PATCH",
            body: JSON.stringify({ id: "uuid-1", doc, format: "rich" }),
        }));
        // The lock-check SELECT runs first; the doc binding lives on the UPDATE call.
        const updateCall = mockQueryOne.mock.calls.find(c => /UPDATE/i.test(c[0] as string));
