...
export async function POST(req: Request) {
    // Owner-only: the local internal flash callers (localhost) pass via the
    // local/LAN bypass; an external hit on the deployed origin is rejected.
    ...
    // Note-triggered flashes (server triggerHue + client fireIntegrations both pass
    // source:"note_created") MUST respect the "Note Created → Hue Flash" automation
    // toggle. Without this gate the flash fires regardless of the switch, because this
    // route otherwise only checks the INTEGRATION's active/mode. Manual/other callers
    // omit `source` and are unaffected.
    if (body.source === "note_created") {
        ...
        if (autom && autom.active === false) return NextResponse.json({ ok: false, reason: "automation off" });
    }
